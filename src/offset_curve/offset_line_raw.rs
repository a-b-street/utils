use super::{line_measured::LineMeasured, vector_extensions::VectorExtensions};
use geo::{Coord, CoordFloat, Line};
...
///
///
pub(super) fn offset_line_raw<T>(a: Coord<T>, b: Coord<T>, distance: T) -> Option<LineMeasured<T>>
where
    T: CoordFloat,
{
    let ab = b - a;
...
mod test {
    ...
    use geo::{
        offset_curve::{line_measured::LineMeasured, offset_line_raw::offset_line_raw},
        Coord, Line,
    };
    ...
    fn test_offset_line_raw() {
        ...
        let output_expected = Some(LineMeasured {
            line: Line {
                start: Coord { x: 1f64, y: 0f64 },
                end: Coord { x: 1f64, y: 1f64 },
            },
            length: 1f64,
        });
