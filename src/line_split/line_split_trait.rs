...
/// Defines functions to split a [Line](geo::Line) or [LineString](geo::LineString)
pub trait LineSplit<Scalar>
where
    Self: Sized,
    Scalar: CoordFloat,
{
    /// Split a [Line](geo::Line) or [LineString](geo::LineString) at some `fraction` of its length.
    ///
    ...
    ///
    /// Otherwise returns [`Some(LineSplitResult)`](geo::algorithm::LineSplitResult)
    ///
    /// example
    ///
    /// ```
